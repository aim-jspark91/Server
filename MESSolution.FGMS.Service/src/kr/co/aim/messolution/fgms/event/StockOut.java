...
public class StockOut extends SyncHandler {
	...
	public Object doWorks(Document doc) throws CustomException {
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("StockOut", getEventUser(), getEventComment(), "", "");
		
		ShipRequest shipRequestData = FGMSServiceProxy.getShipRequestService().selectByKey(false,  new Object[] {invoiceNo});
		
		shipRequestData.setShipRequestState("Shipped");
		...
		{
			String palletName = SMessageUtil.getChildText(elePallet, "DURABLENAME", true);
			...
			udfs.put("LOCATION","");
			udfs.put("OLDAREA", oldProcessGroupData.getUdfs().get("LOCATION"));
			...
			{				
				...
				boxUdfs.put("STOCKSTATE", "Shipped");
				boxUdfs.put("OLDAREA", boxData.getUdfs().get("LOCATION"));
				boxUdfs.put("LOCATION","");
			...
			//reduce Area pallet
			String location = oldProcessGroupData.getUdfs().get("LOCATION");
