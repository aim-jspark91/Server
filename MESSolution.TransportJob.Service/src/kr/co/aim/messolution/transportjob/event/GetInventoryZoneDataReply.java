...
//				} catch(Exception e) {
//					// Modified by smkang on 2018.05.04 - Although any zone has problem in for loop, another zones should be updated.
...
//		
//		// --------------------------------------------------------------------------------------------------------------------------------------------------------
//		// Added by smkang on 2018.08.12 - Need to delete a zone information which is not existed.
...
//		}
//		
//		String condition = "MACHINENAME = ? AND ZONENAME NOT IN (" + StringUtils.removeEnd(StringUtils.repeat("?,", zoneElementList.size()), ",") + ")";
//		List<StockerZoneInfo> removedStockerZoneList = ExtendedObjectProxy.getStockerZoneInfo().select(condition, bindSet);
//		
//		for (StockerZoneInfo stockerZoneInfo : removedStockerZoneList) {
//			ExtendedObjectProxy.getStockerZoneInfo().remove(eventInfo, stockerZoneInfo);
//		}
//		// --------------------------------------------------------------------------------------------------------------------------------------------------------
//		
//		// Added by smkang on 2018.04.14 - Need to forward reply to OPI.
	...
	public void doWorks(Document doc) throws CustomException {
		try {
			...
			for(Element zoneElement : zoneElementList) {
				...
				//if (totalCapacity > 0 && (usedShelfCount + emptyShelfCount+ prohibitedShelfCount) == totalCapacity) { //add by GJJ mantis 5208
					if (totalCapacity > 0 ) { //modify byjhying  on 20200311 mantis :5758,5769
					try {
