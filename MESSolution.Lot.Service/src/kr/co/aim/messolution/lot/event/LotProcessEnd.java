...
public class LotProcessEnd extends SyncHandler
{
		...
		throws CustomException
	{
		...
		try
		{
			SMessageUtil.setHeaderItemValue(doc, "MESSAGENAME", "A_LotProcessEndReply");
			...
			MESLotServiceProxy.getLotServiceImpl().insertCT_ProductProcessOperation(productElement, eventInfo);
    		...
    		else
    		{
    		    ...
    		    catch (CustomException ce)
    	        {
    	            ...
    	            //MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","ABND","", false, "A_LotProcessEnd");
    	            /* 20190426, hhlee, modify, add variable(setFutureHold) */
    	            //MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","ABND","", false, "A_LotProcessEnd", machineSpecData.getUdfs().get("DEPARTMENT"));
    	            try {
    	            	GenericServiceProxy.getTxDataSourceManager().beginTransaction();
    	            	MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","ABND","", false, false, "A_LotProcessEnd", machineSpecData.getUdfs().get("DEPARTMENT"));
    	        ...
    	        EventInfo trackOutLotEventInfo = EventInfoUtil.makeEventInfo("", getEventUser(), getEventComment(), null, null);
    	        /* <<== 20180920, hhlee, add, Modify EventName, EventComment */
    			...
    			//List<ProductPGSRC> productPGSRCSequence = MESLotServiceProxy.getLotInfoUtil().setProductPGSRCSequence(productElement, machineName);
    			...
    			//2018.12.21 dmlee : Add Hold Validation (Mantis : 2072)
    			...
    			try
    			{
    			    for (Element product : productElement )   
    		        {
                        ...
                        if(StringUtil.isNotEmpty(machineRecipeName))
                        {
                            break;
                        }
    		        }			    
    			...
    			// check FirstFlag by RecipeIdleTime. if lot is firstFlag, reserve AHold in FutureAction.
    			...
    			// -----------------------------------------------------------------------------------------------------------------------------------------------------------
    	   		...
    	   		try {
    	   	   		// MQC Job俊 积己等 傍沥甸苞 Lot 狼 ProcessFlow 狼 傍沥甸捞 促福促搁 aHoldFlag 甫 true 肺 汲沥
    	   			aHoldFlag = MESLotServiceProxy.getLotServiceImpl().getAholdFlagMQCOperationListAndProcessFlowOperationList(lotData.getKey().getLotName(),lotData.getProcessFlowName());
    			} catch (Exception e) {
    				
    			}
    	   		if(!aHoldFlag){
        	   		aHoldFlag = MESLotServiceProxy.getLotServiceUtil().isExistAhold(lotData.getKey().getLotName(), lotData.getProcessFlowName()
    			...
    			for (Element product : productElement )
                {
    			    ...
    			    /* <<== 20181225, hhlee, change, not used for loop */
    			    
    			    /* 20181225, hhlee, add, add PRODUCT_PROCESSINGINFO_W ==>> */
    			...
    			Lot afterTrackOutLot = MESLotServiceProxy.getLotServiceUtil().trackOutLot(eventInfo, lotList.get(0), portData,
    													...
    													productPGSRCSequence, assignCarrierUdfs, deassignCarrierUdfs, decideSampleNodeStack, aHoldFlag,note.toString());
    			// Track Out Lot****************************************************************************************
    	        // *****************************************************************************************************
    	        // *****************************************************************************************************
    			...
    			if(StringUtil.equals(elaQtimeFlag, GenericServiceProxy.getConstantMap().Flag_Y))
    			{
    			    ...
    			    /* 20190426, hhlee, modify, add variable(setFutureHold) */
    			    //MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","ELAH","", false, "A_LotProcessEnd", " ");
    			    try {
    			    	GenericServiceProxy.getTxDataSourceManager().beginTransaction();
    			    	MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","ELAH","", false, false, "A_LotProcessEnd", "");
    			    	GenericServiceProxy.getTxDataSourceManager().commitTransaction();
					} catch (Exception e) {
						GenericServiceProxy.getTxDataSourceManager().rollbackTransaction();
					}
                ...
                // Start 2019.12.10 Add By Park Jeong Su For after MoveToEndBank modify HoldState Probleam
                afterTrackOutLot = MESLotServiceProxy.getLotInfoUtil().getLotData(afterTrackOutLot.getKey().getLotName());
                // End 2019.12.10 Add By Park Jeong Su For after MoveToEndBank modify HoldState Probleam
    			...
    			//2019.05.10 dmlee : Get MQCCheck Before ProcessFlow
    			ProcessFlowKey processFlowKeyMQC = new ProcessFlowKey();
    			processFlowKeyMQC.setFactoryName(afterTrackOutLot.getFactoryName());
    			processFlowKeyMQC.setProcessFlowName(beforeProcessFlowName);
    			processFlowKeyMQC.setProcessFlowVersion("00001");
    			ProcessFlow processFlowDataMQC = ProcessFlowServiceProxy.getProcessFlowService().selectByKey(processFlowKeyMQC);
    
    			if(processFlowDataMQC != null)
    			{
    				if(StringUtils.equals(processFlowDataMQC.getProcessFlowType(), "MQC"))
    				{
    					try {
    					...
    					eventInfo.setEventTimeKey( TimeUtils.getCurrentEventTimeKey());
    					MESProductServiceProxy.getProductServiceImpl().updateMQCCountToProduct(afterTrackOutLot, eventInfo, processFlowDataMQC, beforeProcessOperationName);
    					...
    					if(StringUtil.equals(afterTrackOutLot.getLotState(), "Completed"))
    					{
    						...
    						eventInfo.setEventTime( TimeStampUtil.getCurrentTimestamp());
    						eventInfo.setEventTimeKey( TimeUtils.getCurrentEventTimeKey());
    						MESProductServiceProxy.getProductServiceImpl().checkFinishMQCJob(afterTrackOutLot, eventInfo, processFlowDataMQC);
    	        ...
    	        catch (Exception ex) 
    	        {
                    ...
                    //        "HoldLot", "ABND", StringUtil.EMPTY, false, "A_LotProcessEnd", StringUtil.EMPTY);
                    try {
                    	GenericServiceProxy.getTxDataSourceManager().beginTransaction();
                        MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName, null, 
                                "HoldLot", "ABND", StringUtil.EMPTY, false, false, "A_LotProcessEnd", StringUtil.EMPTY);
                        GenericServiceProxy.getTxDataSourceManager().commitTransaction();
					} catch (Exception e) {
						GenericServiceProxy.getTxDataSourceManager().rollbackTransaction();
					}
    	      ...
    	      //end by jhying on20200303 mantis:5435
    			...
    			try
    			{
    				//2019.10.23 dmlee : 'A_LotProcessEndReply' No Regist ClassMap in FMCsvr, If you want, Regist Class Map.
	...
	private void sortProcessEnd(Document doc) throws CustomException
	{
		...
		{
			throw new CustomException("SORT-0001", carrierName);
		}
        ...
        /* 20181112, hhlee, add, Lot is hold, If the quantity of productElement and LotProductList is not same ==>> */
        boolean isDifferentProductQuantity = false;
        String productElementSize = StringUtil.EMPTY;
        String productMesSize = StringUtil.EMPTY;
        /* <<== 20181112, hhlee, add, Lot is hold, If the quantity of productElement and LotProductList is not same */
...
//		                trackOutLot = MESLotServiceProxy.getLotInfoUtil().getLotData(trackOutLot.getKey().getLotName());
			...
			{
			    /* 20181112, hhlee, add, ProductElement Size Validation ==>> */
			    ...
			    if(productDataList.size() != productElement.size())
			    {
			        isDifferentProductQuantity = true; 
			        productElementSize = String.valueOf(productElement.size());
			        productMesSize = String.valueOf(productDataList.size());
			    }			    
			    /* <<== 20181112, hhlee, add, Lot is hold, If the quantity of productElement and LotProductList is not same */
			}			
		}
...
//    								targetLot = MESLotServiceProxy.getLotInfoUtil().getLotData(lotList.get(0).getKey().getLotName());
    								targetLot = LotServiceProxy.getLotService().selectByKeyForUpdate(lotList.get(0).getKey());
    							}			
    						}
		...
		else if(StringUtil.equals(detailSortJobType, GenericServiceProxy.getConstantMap().SORT_JOBTYPE_TURNOVER))
		{
			...
			for (Element product : productElement )
			{
				...
				if(StringUtil.equals(processingInfo, GenericServiceProxy.getConstantMap().PRODUCT_PROCESSINGINFO_P))
				{
				    ...
				    //setLotInfoReceiveProductFlagByTurn
					//ExtendedObjectProxy.getProductFlagService().setLotInfoReceiveProductFlag(eventInfo, product, GenericServiceProxy.getConstantMap().PRODUCTFLAGUPLOAD);
				    ExtendedObjectProxy.getProductFlagService().setLotInfoReceiveProductFlagByTurn(eventInfo, product, GenericServiceProxy.getConstantMap().PRODUCTFLAGUPLOAD);
...
//                    trackOutLot = MESLotServiceProxy.getLotInfoUtil().getLotData(sourceLotData.getKey().getLotName());
                    trackOutLot = LotServiceProxy.getLotService().selectByKeyForUpdate(sourceLotData.getKey());
                    /* <<== 20190114, hhlee, modify, Change Logic SourceLot TrackOut */
                }
                /* <<== 20181205, hhlee, add, add SourceLot Valiable */
 			}			
    		...
    		else
    		{
    			...
    			else
    			{
        			...
        			if(nodeStackList.length > 0)
        			{
                        ...
                        String returnProcessFlowVersion = "";
                        String returnProcessOperationName = "";
                        String returnProcessOperationVersion = "";
                        
                        if(StringUtil.equals(trackOutLot.getUdfs().get("BEFOREOPERATIONNAME"),"-"))
                        {
                        	returnProcessFlowName = trackOutLot.getUdfs().get("BEFOREFLOWNAME");
            				returnProcessFlowVersion = GenericServiceProxy.getConstantMap().DEFAULT_ACTIVE_VERSION;
            				returnProcessOperationName = trackOutLot.getProcessOperationName();
            				returnProcessOperationVersion = GenericServiceProxy.getConstantMap().DEFAULT_ACTIVE_VERSION;
                        }
                        else
                        {
                        	returnProcessFlowName = nodeData.getProcessFlowName();
            				returnProcessFlowVersion = GenericServiceProxy.getConstantMap().DEFAULT_ACTIVE_VERSION;
            				returnProcessOperationName = nodeData.getNodeAttribute1();
            				returnProcessOperationVersion = GenericServiceProxy.getConstantMap().DEFAULT_ACTIVE_VERSION;
                        }
        		        ...
        		        {
        					if(StringUtil.equals(nodeStackList[nodeStackList.length - 1], "111111111111111"))
        					{
        						nextNode = ProcessFlowServiceProxy.getProcessFlowService().getNode("111111111111111");
        					}
        					else
        					{
                        ...
                        ChangeSpecInfo changeSpecInfo = MESLotServiceProxy.getLotInfoUtil().changeSpecInfo(trackOutLot.getKey().getLotName(),
                                ...
                                trackOutLot.getDueDate(), trackOutLot.getPriority(), trackOutLot.getFactoryName(), trackOutLot.getAreaName(),
                                trackOutLot.getLotState(), trackOutLot.getLotProcessState(), trackOutLot.getLotHoldState(),
                                returnProcessFlowName, returnProcessFlowVersion, returnProcessOperationName, returnProcessOperationVersion,
                                "", "", "", "", "", trackOutLot.getUdfs(), productUdfs, false);
        				/* <<== 20181023, hhlee, add, When EndBank Sort, LotState = 'Completed' and ProcessOperationName = '-' and ProcessOperationVersion = ""  */
                        ...
                        // 2019.06.04_hsryu_Delete Logic. Not Memory TrackOut Event. Memory New Event. Mantis 0003934.
...
//        				trackOutLot = LotServiceProxy.getLotService().selectByKey(trackOutLot.getKey());
                        ...
                        //2019.02.13_hsryu_Move to Logic location.
                        boolean aHoldFlag = false;
                        ...
                        /* 20181112, hhlee, add, ProductElement Size Validation ==>> */
                        if(isDifferentProductQuantity)
                        {
                        	aHoldFlag = true;
                            eventInfo.setEventName("SorterAbnormalEndHold");
                            eventInfo.setEventComment("Changer has been Finished Please Check![Receive Product Count : " + productElementSize + "MES Product Count : " + productMesSize + "]");
                            /* 20190424, hhlee, modify, changed function ==>> */
                            //MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot", "STOH", "", "MFG");
                            /* 20190426, hhlee, modify, add variable(setFutureHold) */
                            //MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo, carrierName,null,"HoldLot","STOH","", true, "", "MFG");
...
//                        trackOutLot = MESLotServiceProxy.getLotInfoUtil().getLotData(trackOutLot.getKey().getLotName());
		...
		/* 20190102, hhlee, add, Add Logic SorterJobEnd Validation ==>> */
		try
        {
        ...
        {
            eventLog.warn(String.format("This sorterjob[%s] is not ended.!", sortJobName));
        } 
...
//		return MESLotServiceProxy.getLotInfoUtil().getLotData(targetLot.getKey().getLotName());
	...
	private void UnpackerProcessEnd(Document doc) throws CustomException
	{
		String machineName = SMessageUtil.getBodyItemValue(doc, "MACHINENAME", true);
		String carrierName = SMessageUtil.getBodyItemValue(doc, "CARRIERNAME", true);
		String portName = SMessageUtil.getBodyItemValue(doc, "PORTNAME", true);
...
//		Durable durableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(carrierName);
		...
		try
		{
    		...
    		////errorCode:
    		////[CRATE-0011],[PRODUCTREQUEST-0022],[PRODUCTREQUEST-0053],[PRODUCTREQUEST-0002],[LOT-0207],[PRODUCTREQUEST-0021]
...
//        			pPlanData = ProductRequestPlanServiceProxy.getProductRequestPlanService().selectByKey(pPlanKey);
    		...
    		String productListProcessingInfoF = "";//add BY GJJ 20200515 unpak机台上报 W/F 需要Hold mantis:6154
    		...
    		for(Element ePruduct : eProductListElement)
    		{
              ...
              //add BY GJJ 20200515 unpak机台上报 W/F 需要Hold mantis:6154 start
                String processingInfo =  SMessageUtil.getChildText(ePruduct, "PROCESSINGINFO", false); 
                
    		    if(StringUtil.equals(processingInfo, GenericServiceProxy.getConstantMap().PRODUCT_PROCESSINGINFO_F))
                {
                    if(StringUtil.isEmpty(productListProcessingInfoF))
                    {
                       productListProcessingInfoF = sProductName + "-->ProcessInfo = F";
                    }
                    else
                    {
                        productListProcessingInfoF += "," + sProductName + "-->ProcessInfo = F";
    		    ...
    		    else if (StringUtil.equals(processingInfo, GenericServiceProxy.getConstantMap().PRODUCT_PROCESSINGINFO_W))
    		    {
                    if(StringUtil.isEmpty(productListProcessingInfoF))
                    {
                       productListProcessingInfoF = sProductName + "-->ProcessInfo = W";
                    }
                    else
                    {
                        productListProcessingInfoF += "," + sProductName + "-->ProcessInfo = W";
    		  ...
    		  //add BY GJJ 20200515 unpak机台上报 W/F 需要Hold mantis:6154 end
    		...
    		//1.3)Release Lot
    		//[LOT-9001],[LOT-9002],[LOT-9003],[LOT-9999]
			...
			if(StringUtil.isNotEmpty(productListProcessingInfoF))
            {
				EventInfo eventInfo1 = EventInfoUtil.makeEventInfo("Hold", getEventUser(), getEventComment(), "", "");
                eventInfo1.setEventComment(String.format("LotName[%s]( %s) ", 
                		lotData.getKey().getLotName(), productListProcessingInfoF));
                MachineSpec  machineSpecData = MESMachineServiceProxy.getMachineInfoUtil().getMachineSpec(machineName);
                try {
                	GenericServiceProxy.getTxDataSourceManager().beginTransaction();
                	//MESLotServiceProxy.getLotServiceUtil().doAfterHoldbyCarrier(eventInfo1, carrierName,lotData,"HoldLot","PFHL","", true, false, "", machineSpecData.getUdfs().get("DEPARTMENT"));
                	MESLotServiceProxy.getLotServiceUtil().futureHoldLot(eventInfo1, lotData, eventInfo.getEventUser(), "AHOLD", "PFHL");
                	GenericServiceProxy.getTxDataSourceManager().commitTransaction();
				} catch (Exception e) {
					GenericServiceProxy.getTxDataSourceManager().rollbackTransaction();
				}
...
//    		List<Product> productList = MESProductServiceProxy.getProductServiceUtil().getProductListByLotName(lotData.getKey().getLotName());
    		...
    		if(lotData.getProductQuantity() != lotCreateProductQty)
    		{
    			...
    			/* hhlee, 20180703, Add Unpacker LotProcessEnd Abnormal Hold ==>> */
    			//futureHoldLot(lotData, machineName, eventInfo);
    			eventInfo.setEventComment("Lot Create Product Quantity and Product Quantity are different");
    			//MESLotServiceProxy.getLotServiceUtil().futureHoldLot(lotData, machineName, eventInfo, "HoldLot", "ABND");
    			isLotHold = true;
    			/* <<== hhlee, 20180703, Add Unpacker LotProcessEnd Abnormal Hold */
    		}
...
//                //if ( StringUtil.equals(lotList.get(0).getUdfs().get("SUPERLOTFLAG"), "N") ||
//                //       StringUtil.isEmpty(lotList.get(0).getUdfs().get("SUPERLOTFLAG")) )
//                //{
//                //  // Decide Sampling
//                //  decideSampleNodeStack = MESLotServiceProxy.getLotServiceUtil().getDecideSample(doc, eventInfo, lotList.get(0));
//                //}
//                //
//                //if ( (StringUtil.equals(lotList.get(0).getUdfs().get("SUPERLOTFLAG"), "N") || StringUtil.isEmpty(lotList.get(0).getUdfs().get("SUPERLOTFLAG")) ) 
//                //      && StringUtil.isNotEmpty(MESLotServiceProxy.getLotServiceUtil().checkMoveToSample(lotList.get(0))))
//                //{
...
//                //      MESMachineServiceProxy.getMachineServiceImpl().updateMachineIdleTimeRunInfo(machineName, unitSpec.getKey().getMachineName(), lotData, eventInfo);
//                //  }
//                //} catch (Exception e) {
//                //  eventLog.warn(e);
//                //}
...
//                //        productPGSRCSequence, assignCarrierUdfs, deassignCarrierUdfs, decideSampleNodeStack, aHoldFlag);
//                
//                
//                /* 20190419, hhlee, delete, Unnecessary Logic ==>> */
//                ////Port clear - YJYU
...
//                
//                // 2019.05.29_hsryu_Move to Logic. if LotState is Completed, Change 'ProcessOperationName' in TrackOutLot Function. 
...
//                    Map<String, String> productUdfs = productData.getUdfs();
//                    
//                    //if((StringUtil.isEmpty(productUdfs.get("TURNOVERFLAG"))|| StringUtil.equals(productUdfs.get("TURNOVERFLAG"), GenericServiceProxy.getConstantMap().Flag_N)) &&
...
//                    Map<String, String> productUdfs = productData.getUdfs();
//                    
//                    //if((StringUtil.isEmpty(productUdfs.get("TURNSIDEFLAG")) || StringUtil.equals(productUdfs.get("TURNSIDEFLAG"), GenericServiceProxy.getConstantMap().Flag_N)) &&
...
//                        //}
//                        
//                        /* 20190212, hhlee, add, product state update(ProductState = 'Idle') ==>> */
//                        //MESProductServiceProxy.getProductServiceImpl().productStateUpdateBySortEnd(eventInfo, trackOutLot);
//                        /* <<== 20190212, hhlee, add, product state update(ProductState = 'Idle') */                    
//                        /* <<== 20180810, Add, Update Product - ProductProcessState[Processing -> Idle] */
//                        
...
////     * @param EventName
////     */
////    private void copyLotHoldCallSplitMerge(Lot parentLot, Lot childLot, EventInfo eventInfo, String EventName)
////    {
////        try
////        {
////            eventInfo.setEventName(GenericServiceProxy.getConstantMap().SORT_JOBTYPE_SPLIT);
...
////     * @throws CustomException
////     */
////    private void copyLotHoldForSplitMerge(Lot sLot, Lot dLot, EventInfo eventInfo) throws CustomException
////    {
////        
...
////     * @return
////     */
////    private String setActualSamplePosition(EventInfo eventInfo, Lot lotData, String logicalSlotMap)
////    {
////        String actualSamplePosition = StringUtil.EMPTY;
