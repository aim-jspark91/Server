...
public class MQCSheetSetting extends SyncHandler{
	@Override
	public Object doWorks(Document doc) throws CustomException 
	{
		...
		//String recipelist = SMessageUtil.getBodyItemValue(doc, "RECIPELIST", true);
...
//		String temprecipeName = SMessageUtil.getBodyItemValue(doc, "RECIPEID", false);
		...
		{
			if(StringUtil.equals(mqcTemplateName , ""))		{
				...
				mqcTemplateName = tempmqcTemplateName.get(0);
			}
		}
		...
		for(Element elePosition : elePositionList)
		{
					...
					for( int k=1; k< newsqlResult.size(); k++)
					{
						...
						{							
							if(StringUtil.equals(newsqlResult.get(k).get("PROCESSOPERATIONNAME").toString(), processOperationName))
							{
								...
								boolean result = SettingPosition(mqcTemplateName, newsqlResult.get(k).get("PROCESSOPERATIONNAME").toString(), processOperationVersion, Long.valueOf(position), StringUtils.EMPTY, "" , eventInfo, processflag);
							}
						}
					}
				}
			...
			{
				boolean result = SettingPosition(mqcTemplateName, processOperationName, processOperationVersion, Long.valueOf(position), recipeName, mqcCountUp, eventInfo, processflag);	
			}		
		}
			
	    SMessageUtil.addItemToBody(doc, "NEWMQCTEMPLATENAME", mqcTemplateName);
		return doc;
	}
