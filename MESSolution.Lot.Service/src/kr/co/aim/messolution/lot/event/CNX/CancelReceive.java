...
public class CancelReceive extends SyncHandler {
	@Override
		...
		throws CustomException
	{
		...
		List<Element> eCSTList = SMessageUtil.getBodySequenceItemList(doc, "CSTLIST", true); 
		
		String arrDBUserName = ((BasicDataSource)GenericServiceProxy.getSqlMesTemplate().getDataSource()).getUsername();
		arrDBUserName = arrDBUserName.replaceAll("oled", "array");
		
		
		StringBuilder sql = new StringBuilder();
		Map<String, Object> args = new HashMap<String, Object>();
		sql.append(" DELETE FROM CT_CANCEL_RECEIVEDLOT_TEMP " );
		...
		{	
			String durableName = SMessageUtil.getChildText(eCarrier, "DURABLENAME", true);
			List<Lot> lotList = LotServiceProxy.getLotService().select("WHERE CARRIERNAME = ? ", new Object[]{durableName});
			
			if(lotList.size() > 0)
			{	
				//String originalLotName = lotList.get(0).getOriginalLotName();
				for(Lot lotData : lotList)
				{
					
					if(!this.validateOLEDLotData(lotData.getKey().getLotName(), "OLED"))  // oled DB에 lotState가 'Received'인 녀석들이 있는가
					{
						throw new CustomException("LOT-0244");
					}
					
					//20180504, kyjung, QTime
					MESProductServiceProxy.getProductServiceImpl().ExitedCancelFabQTime(eventInfo, lotData, "Receive", lotData.getDestinationFactoryName());
					
					this.ReleaseHold(lotData,eventInfo);
					
					sql = new StringBuilder();
					sql.setLength(0);
					sql.append(" INSERT INTO CT_CANCEL_RECEIVEDLOT_TEMP ( LOTNAME,ORIGINALLOTNAME,CARRIERNAME, RECEIVEBANK  )" );
					sql.append(" VALUES (  :LOTNAME, :ORIGINALLOTNAME, :CARRIERNAME, :RECEIVEBANK )" );
					args = new HashMap<String, Object>();
					args.put("LOTNAME",lotData.getKey().getLotName());
					args.put("ORIGINALLOTNAME", lotData.getOriginalLotName() );
					args.put("CARRIERNAME",durableName);
					args.put("RECEIVEBANK", "");
		...
		GenericServiceProxy.getSqlMesTemplate().executeProcedure("P_MAKE_CANCEL_RECEIVED_DATA");
		///////////////////////////////////////////////////////////////////////////////////////////
		
		/*for (Element eProduct : eCSTList)
		{	
			String durableName = SMessageUtil.getChildText(eProduct, "DURABLENAME", true);
			List<Lot> lotList = LotServiceProxy.getLotService().select("WHERE CARRIERNAME = ? ", new Object[]{durableName});
			...
			{		
				...
				//Modified by jjyoo on 20180829
				String deleteLotSql = "DELETE FROM LOT WHERE LOTNAME = ?";
				
				String deleteLotHistSql= "INSERT INTO LOTHISTORY (LOTNAME, TIMEKEY, EVENTTIME, EVENTNAME, PRODUCTIONTYPE, PRODUCTSPECNAME, PRODUCTSPECVERSION, PROCESSGROUPNAME, PRODUCTREQUESTNAME, ORIGINALLOTNAME, SOURCELOTNAME, DESTINATIONLOTNAME,"
						+ "ROOTLOTNAME, PARENTLOTNAME, CARRIERNAME, PRODUCTTYPE, SUBPRODUCTTYPE, SUBPRODUCTUNITQUANTITY1, "
						+ "PRODUCTQUANTITY, SUBPRODUCTQUANTITY, SUBPRODUCTQUANTITY1, SUBPRODUCTQUANTITY2, LOTGRADE, DUEDATE, PRIORITY, FACTORYNAME, DESTINATIONFACTORYNAME, AREANAME, LOTSTATE, LOTPROCESSSTATE, LOTHOLDSTATE,"
						+ "PROCESSFLOWNAME, PROCESSFLOWVERSION, PROCESSOPERATIONNAME, PROCESSOPERATIONVERSION, REWORKSTATE, NOTE, ECCODE, SHIPBANK, RECEIVEBANK, EVENTUSER, "
						+ "OLDPRODUCTIONTYPE, OLDPRODUCTSPECNAME, OLDPRODUCTSPECVERSION, OLDPRODUCTSPEC2NAME, OLDPRODUCTSPEC2VERSION, OLDPRODUCTTYPE, OLDSUBPRODUCTTYPE, OLDPRODUCTQUANTITY, OLDSUBPRODUCTQUANTITY,"      
						+ "OLDSUBPRODUCTQUANTITY1, OLDSUBPRODUCTQUANTITY2, OLDFACTORYNAME, OLDDESTINATIONFACTORYNAME, OLDAREANAME, OLDPROCESSFLOWNAME, OLDPROCESSFLOWVERSION, OLDPROCESSOPERATIONNAME, OLDPROCESSOPERATIONVERSION, OLDPRODUCTREQUESTNAME)"
						+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				
				String deleteProductSql = "DELETE FROM PRODUCT WHERE PRODUCTNAME = ?";
				
				String deleteProductHistSql = "INSERT INTO PRODUCTHISTORY (PRODUCTNAME, TIMEKEY, EVENTTIME, EVENTNAME, OLDPRODUCTIONTYPE, OLDPRODUCTSPECVERSION, OLDLOTNAME, OLDSUBPRODUCTTYPE, OLDSUBPRODUCTQUANTITY, OLDSUBPRODUCTQUANTITY1, OLDSUBPRODUCTQUANTITY2,"
						+ "OLDFACTORYNAME, OLDDESTINATIONFACTORYNAME, OLDAREANAME, OLDPROCESSFLOWNAME, OLDPROCESSFLOWVERSION, OLDPROCESSOPERATIONNAME, OLDPROCESSOPERATIONVERSION,"
						+ "PRODUCTIONTYPE, PRODUCTSPECNAME, PRODUCTSPECVERSION, PRODUCTREQUESTNAME, ORIGINALPRODUCTNAME, LOTNAME, CARRIERNAME, POSITION, PRODUCTTYPE,"
						+ "SUBPRODUCTTYPE, SUBPRODUCTUNITQUANTITY1, SUBPRODUCTUNITQUANTITY2, SUBPRODUCTQUANTITY, SUBPRODUCTQUANTITY1,"
						+ "SUBPRODUCTQUANTITY2, PRODUCTGRADE, DUEDATE, PRIORITY, FACTORYNAME, DESTINATIONFACTORYNAME, AREANAME, PRODUCTSTATE,"
						+ "PROCESSFLOWNAME, PROCESSFLOWVERSION, PROCESSOPERATIONNAME, PROCESSOPERATIONVERSION, NODESTACK, REWORKSTATE, REWORKCOUNT, NOTE, ECCODE )"
						+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				
				List<Object[]> deleteLotArgList = new ArrayList<Object[]>();
				List<Object[]> deleteLotHistArgList = new ArrayList<Object[]>();
				List<Object[]> deleteProductArgList = new ArrayList<Object[]>();
				List<Object[]> deleteProductHistArgList = new ArrayList<Object[]>();
			
				for(Lot lotData : lotList)
				{
					//20180504, kyjung, QTime
					MESProductServiceProxy.getProductServiceImpl().ExitedCancelFabQTime(eventInfo, lotData, "Receive");
					
					eventInfo.setEventName("CancelReceive");
					String deleteLotName = lotData.getKey().getLotName();
					String deleteProductName = lotData.getKey().getLotName();
					
					List<Object> lotNameBindList = new ArrayList<Object>();
					lotNameBindList.add(lotData.getKey().getLotName());
					
					List<Object> productNameBindList = new ArrayList<Object>();
					productNameBindList.add(lotData.getKey().getLotName());
					
					//Lot
					deleteLotArgList.add(lotNameBindList.toArray());
					deleteLotHistArgList.add(this.getLotHistInsertArgList(eventInfo, deleteLotName, deleteProductName, lotData.getFactoryName(),lotData.getProductSpecName()).toArray());
					
					//Product
					deleteProductArgList.add(productNameBindList.toArray());
					deleteProductHistArgList.add(this.getProductHistInsertArgList(eventInfo, deleteProductName, lotData.getFactoryName()).toArray());
					*/
					/*List<ProductU> productUSequence = MESLotServiceProxy.getLotInfoUtil().getAllProductUSequence(lotData);
					kr.co.aim.nanotrack.lot.management.info.SetEventInfo setEventInfo 
									= MESLotServiceProxy.getLotInfoUtil().setEventInfo(lotData, lotData.getProductQuantity(), productUSequence);
					
					eventInfo.setEventName("CancelReceive");
					LotServiceProxy.getLotService().setEvent(lotData.getKey(),eventInfo, setEventInfo);
					LotServiceProxy.getLotService().delete(lotData.getKey());*/
					
					/*============= Delete Product Data =============*/
					/*String productName = lotData.getKey().getLotName();
					Product productData = MESProductServiceProxy.getProductServiceUtil().getProductData(productName);
					
					if(productData != null)
					{	
						kr.co.aim.nanotrack.product.management.info.SetEventInfo setEventInfo 
									= new kr.co.aim.nanotrack.product.management.info.SetEventInfo();
						MESProductServiceProxy.getProductServiceImpl().setEvent(productData, setEventInfo, eventInfo);
						ProductServiceProxy.getProductService().delete(productData.getKey());
					}*/
					
					/*
					//2018.07.20 dmlee : remove HQJudge,PanelJudge Data
					try
					{
						this.deleteHQGlassJudge(eventInfo, deleteProductName);
					}
					catch(Exception ex)
					{
					...
					try
					{
						this.deletePanelJudge(eventInfo, deleteProductName);
					}
					catch(Exception ex)
					{
						eventLog.error("Panel Judge Data Remove Fail ! ");
						throw new CustomException("LOT-0237", "");
					}
					//2018.07.20 dmlee : ------------------------------
					...
					try
					{
						this.updateOnlyArrayProductData(deleteLotName,arrDBUserName);
					}
					catch(Exception ex)
					{
				...
				//UpdateBatch Lot, History
				GenericServiceProxy.getSqlMesTemplate().updateBatch(deleteLotSql, deleteLotArgList);
				GenericServiceProxy.getSqlMesTemplate().updateBatch(deleteLotHistSql, deleteLotHistArgList);
				
				//UpdateBatch Product, History
				GenericServiceProxy.getSqlMesTemplate().updateBatch(deleteProductSql, deleteProductArgList);
				GenericServiceProxy.getSqlMesTemplate().updateBatch(deleteProductHistSql, deleteProductHistArgList);
				*/				
				/*============= Delete Durable Data =============*/ /*
				Durable durableData = MESDurableServiceProxy.getDurableServiceUtil().getDurableData(durableName);
				if(durableData != null)
				{
					kr.co.aim.nanotrack.durable.management.info.SetEventInfo setEventInfo 
								= MESDurableServiceProxy.getDurableInfoUtil().setEventInfo(durableData.getUdfs());
					
					MESDurableServiceProxy.getDurableServiceImpl().setEvent(durableData, setEventInfo, eventInfo);
					DurableServiceProxy.getDurableService().delete(durableData.getKey());				
				}
				
				//Update Array CST Data
				try
				{
					this.updateOnlyArrayLotData(originalLotName, arrDBUserName);
					this.updateOnlyArrayCSTData(durableName, arrDBUserName);
				}
				catch(Exception ex)
				{
					throw new CustomException("LOT-0226","");
	...
	@SuppressWarnings("unchecked")
	private boolean validateOLEDLotData(String lotName, String factoryName) throws CustomException
	{
		try
		{
