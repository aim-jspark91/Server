...
public class PortInInhibit extends SyncHandler
{
	@Override
	public Object doWorks(Document doc) throws CustomException 
	{
		...
		EventInfo eventInfo = EventInfoUtil.makeEventInfo("Change PortInInhibit", this.getEventUser(), this.getEventComment(), "", "");
		...
		for (Element elePort : PortList) 
		{
			...
			if(!StringUtil.equals(portData.getUdfs().get("PORTININHIBITFLAG"), "Y"))
			{
				setEventInfo.getUdfs().put("PORTININHIBITFLAG", "Y");
			}
			else
			{
				setEventInfo.getUdfs().put("PORTININHIBITFLAG", "N");
			}
...
//			eventInfo.setEventComment(this.getEventComment()+" : "+ note);
		...
		// Send FMC
		MESLotServiceProxy.getSendServiceImpl().SendToFMCPortInInhibit(doc);
