 ...
 * @author smkang
 * @since 2018.12.13
 * @see Processing BC alarm by ALMsvr.
 */
public class AlarmReport extends AsyncHandler {

	@Override
	public void doWorks(Document doc) throws CustomException {
		//eventLog.debug(JdomUtils.toString(doc));
		...
		{
			eventInfo = EventInfoUtil.makeEventInfo("Issue", getEventUser(), getEventComment(), "", "");
		}
		else if (StringUtils.equals(alarmState, GenericServiceProxy.getConstantMap().ALARMSTATE_CLEAR))
		{
			eventInfo = EventInfoUtil.makeEventInfo("Clear", getEventUser(), getEventComment(), "", "");
		}
		else
		{
			try
		...
		String[] productNames = new String[productElementList.size()];
		for (int index = 0; index < productElementList.size(); index++)
		{
			productNames[index] = SMessageUtil.getChildText(productElementList.get(index), "PRODUCTNAME", true);
		}
		
		MachineAlarmList machineAlarmData = new MachineAlarmList(machineName, StringUtils.isNotEmpty(unitName) ? unitName : " ", StringUtils.isNotEmpty(subUnitName) ? subUnitName : " ", alarmCode);
		machineAlarmData.setAlarmState(alarmState);
		machineAlarmData.setAlarmSeverity(alarmSeverity);
		machineAlarmData.setAlarmText(alarmText);
		machineAlarmData.setProductList(Arrays.toString(productNames));
		...
		try
		{
          ...
          try
          {
        	  //2019.04.16 dmlee : Machine Alarm No Insert CT_ALARM Table
        	  //MESAlarmServiceProxy.getAlarmServiceImpl().createAlarm(alarmCode, alarmDefinitionData.getAlarmSeverity(), alarmState, alarmDefinitionData.getAlarmType(), alarmDefinitionData.getDescription(), alarmDefinitionData.getFactoryName(), machineName, unitName, subUnitName, machineAlarmData.getProductList(), eventInfo);
		...
		{
          eventLog.info(ex.getLocalizedMessage());
		}
          
        try
        {
	    	  String resultAlarmCode="";
	    	  String resultTimeKey="";
	    	  List<Map<String, Object>> sqlResult = GetAlarmKey("ARRAY",machineName);
	    	  if(sqlResult!=null && sqlResult.size()>0){
	    		  resultAlarmCode=sqlResult.get(0).get("ALARMCODE").toString();
	    		  resultTimeKey=sqlResult.get(0).get("TIMEKEY").toString();
	    	  }
	    	  Alarm alarm = ExtendedObjectProxy.getAlarmService().selectByKey(false, new Object[]{resultAlarmCode,resultTimeKey});
	    	  if(StringUtils.equals(GenericServiceProxy.getConstantMap().ALARMSTATE_CLEAR,alarmState)){
	            MESAlarmServiceProxy.getAlarmServiceImpl().createAlarm(alarm.getAlarmCode(), alarm.getAlarmSeverity(), GenericServiceProxy.getConstantMap().ALARMSTATE_CLEAR,alarm.getAlarmType(), alarm.getDescription(), alarm.getFactoryName(), machineName, unitName, subUnitName, machineAlarmData.getProductList(), eventInfo);
	    	  }
	    }
        catch (Exception e)
        {
	...
	private List<Map<String, Object>> GetAlarmKey(String factoryName,String machineName){
		String sql = "SELECT * " +
				"  FROM CT_ALARM A " +
				" WHERE     1 = 1 " +
				"       AND (A.ALARMCODE, TIMEKEY) IN " +
				"              (SELECT ALARMCODE, TIMEKEY " +
				"                 FROM (SELECT A.ALARMCODE, " +
				"                              A.TIMEKEY, " +
				"                              A.ALARMSTATE, " +
				"                              ROW_NUMBER () " +
				"                              OVER (PARTITION BY A.machinename, A.ALARMTYPE " +
				"                                    ORDER BY A.timekey DESC) " +
				"                                 rn " +
				"                         FROM CT_ALARM A " +
				"                              JOIN MACHINE B " +
				"                                 ON (A.MACHINENAME = B.MACHINENAME) " +
				"                        WHERE B.FACTORYNAME = :FACTORYNAME AND A.ALARMTYPE = :ALARMTYPE AND A.MACHINENAME=:MACHINENAME) " +
				"                WHERE rn = 1 AND ALARMSTATE = :ALARMSTATE) " ;
		Map<String, String> newbindMap = new HashMap<String, String>();
		newbindMap.put("FACTORYNAME", factoryName);
		newbindMap.put("ALARMTYPE", "EQP");
		newbindMap.put("MACHINENAME", machineName);
		newbindMap.put("ALARMSTATE", "ISSUE");
		
		@SuppressWarnings("unchecked")
		List<Map<String, Object>> result = GenericServiceProxy.getSqlMesTemplate().queryForList(sql, newbindMap);
		return result;
	}
