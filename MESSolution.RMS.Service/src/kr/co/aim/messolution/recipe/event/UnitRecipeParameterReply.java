...
public class UnitRecipeParameterReply extends AsyncHandler {
	...
	public void doWorks(Document doc) throws CustomException {
		...
		String oldCompareResultR = null;
		if(eSubUnitList.size() > 0 || recipeparaList.size() > 0)
		{

			recipeData = MESRecipeServiceProxy.getRecipeServiceImpl().createRecipe(eventInfo, unitName, unitRecipeName, GenericServiceProxy.getConstantMap().RECIPE_TYPE_UNIT, newCompareFlag, newCompareResult, seq, "-", newActiveResult, doc);
			
			oldCompareResultR = recipeData.getCompareResult();
		...
		{		
			...
			for (Element eSubUnit : eSubUnitList)
			{
				...
				MESRecipeServiceProxy.getRecipeServiceImpl().createRecipe(eventInfo, subUnitName, subUnitRecipeName, GenericServiceProxy.getConstantMap().RECIPE_TYPE_SUBUNIT, "N", "", "", "-", "NG", doc);
		...
		try
		{
			...
			for(RecipeParameter recipeParam : recipeParamList)
			{
				boolean removeFlag = false;
				
				if(recipeparaList.size() > 0)
				{
					for (Element removeERecipeParam : recipeparaList)
					{
						String recipeParaName = SMessageUtil.getChildText(removeERecipeParam, "RECIPEPARANAME", true);
						String recipeParaValue = SMessageUtil.getChildText(removeERecipeParam, "RECIPEPARAVALUE", false);
						
						if(recipeParam.getMachineName().equals(unitName) && recipeParam.getRecipeName().equals(unitRecipeName) && recipeParam.getRecipeParameterName().equals(recipeParaName))
						{
							removeFlag = false;
							break;
						}
		...
		if(recipeparaList.size() > 0)
		{
			...
			/* Remove Recipe Param */
			MESRecipeServiceProxy.getRecipeServiceImpl().removeRecipeParameter(eventInfo, unitName, unitRecipeName, recipeparaList, doc);
